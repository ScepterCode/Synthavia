...
function eventPage(event) {
  ...
  return `<section class="page-hero dot-grid detail-hero">
    ...
    ${event.gallery?.length ? `<section class="index-section"><p class="eyebrow">RECAP GALLERY</p><h2>What the room<br />looked like.</h2>
      <div class="gallery">${event.gallery.map((photo) => `<figure class="shot"><img src="${esc(photo.src)}" alt="${esc(photo.caption)}" width="${photo.w}" height="${photo.h}" loading="lazy" /><figcaption>${esc(photo.caption)}</figcaption></figure>`).join('')}</div>
